...
class eNews_Updates extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __( 'Displays Feedburner Email Subscription signup form' )
			);
		...
		parent::__construct( 'enews', __( '(SNO) Email Subscription' ), $widget_ops, $control_ops ) ;
	...
	function widget($args, $instance) {
		extract($args); $widget = $this->id; ?>
			<?php $customcolors=$instance['custom-colors']; $categoryslug = ''; $videotitle = ''; $categoryname = '';
			...
			if(!empty($feedburner)) { ?>
			<form id="subscribe" action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $feedburner; ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true"><label class="icon-hidden-text" for="subbox">Enter your email address</label><input type="text" value="Enter your email address..." id="subbox" onfocus="if (this.value == 'Enter your email address...') {this.value = '';}" onblur="if (this.value == '') {this.value = 'Enter your email address...';}" name="email"/><input type="hidden" value="<?php echo $feedburner; ?>" name="uri"/><input type="hidden" name="loc" value="en_US"/><input type="submit" value="GO" id="subbutton" /></form>
			<?php } ?>
...
<div class="hide_all"<?php echo $hide_all; ?>>
		...
		<textarea id="<?php echo $this->get_field_id('text'); ?>" name="<?php echo $this->get_field_name('text'); ?>" style="width: 95%;" rows="6"><?php echo $instance['text']; ?></textarea>
		</p>
