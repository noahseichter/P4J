...
class sno_text extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			'classname' => 'sno_text',
			'description' => __( 'Display arbitrary text or html embeds.' )
			);
		...
		parent::__construct( 'sno_text', __( '(SNO) Text/Embed' ), $widget_ops, $control_ops );
	...
	function widget($args, $instance) {
		extract($args);
			...
			<div class="snotextwidget">			
				...
				<?php if ($instance['paragraphs'] == "Add") { echo wpautop(do_shortcode($instance['text'])); } else { echo do_shortcode($instance['text']); } ?>
			</div>
...
<?php	}
	...
	function update($new_instance, $old_instance) {
		...
		$instance['widget-style'] = $new_instance['widget-style'];
 		...
 		$instance['paragraphs'] = ( isset( $new_instance['paragraphs'] ) ? "Add" : "" );  
...
<div class="hide_all"<?php echo $hide_all; ?>>
		...
		<textarea id="<?php echo $this->get_field_id('text'); ?>" name="<?php echo $this->get_field_name('text'); ?>" style="width: 95%;" rows="9"><?php if (isset($instance['text'])) echo $instance['text']; ?></textarea>
		</p>

		<p>
			<input class="checkbox" type="checkbox" <?php if ($instance['paragraphs'] == "Add") echo 'checked'; ?> id="<?php echo $this->get_field_id( 'paragraphs' ); ?>" name="<?php echo $this->get_field_name( 'paragraphs' ); ?>" />
			<label for="<?php echo $this->get_field_id( 'paragraphs' ); ?>">Automatically Add Paragraphs</label>			
		</p>
