...
/******************Change this*******************/
$api_url = 'http://updates.snosites.com/api/index.php';
/************************************************/
...
add_filter('pre_set_site_transient_update_themes', 'sno_check_for_update');

function sno_check_for_update($checked_data) {
	...
	$send_for_check = array(
		...
		'user-agent' => get_bloginfo('url') . ' | ' . 'WordPress ' . $wp_version
	...
	if (!empty($response)) {
		...
		update_option('snoflexversion', $response[new_version], 'yes');
	}
	return $checked_data;
}

// Take over the Theme info screen on WP multisite
add_filter('themes_api', 'sno_theme_api_call', 10, 3);

function sno_theme_api_call($def, $action, $args) {
