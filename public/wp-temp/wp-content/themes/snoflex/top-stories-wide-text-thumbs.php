...
<?php
echo '<div id="homepagefull" style="position:relative;">';
$loopcounter=0; $featurecount = get_theme_mod('featured-count'); if ($featurecount == '') { $featurecount = 4; }
							echo '<div class="tsthumbstop thumbshide">';
							echo '<div id="tscarousel" class="flexslider">';
  								echo '<ul class="slides">';			
									...
									if (have_posts()) : while (have_posts()) : the_post();
	    								...
	    								$thumb = wp_get_attachment_image_src(get_post_thumbnail_id(), 'carouselthumb', false);
					
										echo '<li class="storyslide" style="max-width:80px;">';
											echo '<img src="' . $thumb[0] . '" alt="' . get_the_title() . '"/>';
										echo '</li>';
			...
			rewind_posts(); if (have_posts()) : while (have_posts()) : the_post();
				...
				$sno_teaser = get_post_meta($post->ID, 'sno_teaser', true);
					...
					echo '<li class="topstorywideslide">';
							...
							<a href="<?php if (($customlink) && (get_theme_mod('top-stories-links')=="Yes")) { echo $customlink; } else { the_permalink(); } ?>#photo"><img src="<?php echo $image[0]; ?>" class="topwideteaserphoto" style="width:50%" alt="<?php the_title(); ?>" /></a> 
							</div>
						<div class="insert">
    								...
    								<?php $categories = get_the_category(); $cat_list = array();
    									...
    									$cat_list = array_filter($cat_list); $co = '';
												if ($cat_list[0] && get_theme_mod('top-stories-hide-cat') == 'No') {
													...
													echo '<div class="topstorycat" style="margin-bottom:5px;">';
